if (typeof(Queuebert) === 'undefined') {
	Queuebert = {};
}

(function() {
	function Client(params) {		
		this.delegate = params.delegate || {};
		this.identifier = params.identifier;
		this.sendRequest = params.sendRequest || chrome.extension.sendRequest;
	}
	
	Client.prototype.start = function() {
		var getMessageInterval = 50,
			_this = this;

		setInterval(function() {
			_this.sendRequest({from: _this.identifier, type: 'get'}, function(action, clientId, body) {
				_this.handleMessage(action, clientId, body);
			}); 
		}, getMessageInterval);
	}
	
	Client.prototype.handleMessage = function(action, clientId, body) {
		if (this.delegate[action]) {
			this.delegate[action](clientId, body);
		}
	};

	Client.prototype.sendMessage = function(msg) {
		
		this.sendRequest({
			to: msg.to,
			from: this.identifier,
			action: msg.action,
			type: 'put',
			body: msg.body
		});
	};

	Queuebert.Client = Client;
})();

// Specifically for our unit tests which are written in Node.js.
if (typeof(exports) !== 'undefined') {
	exports.Client = Queuebert.Client;
}
