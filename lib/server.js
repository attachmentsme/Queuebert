...
(function() {
	function Server(_params) {
		...
		this.messages = {};
		this.maxQueueSize = params.maxQueueSize || 128;
	...
	Server.prototype.handleMessage = function(request, sender, callback) {
		...
		this.registerClient(request.from, tabId);
		...
		if (request.type == 'put') {
			...
			this.messages[ tabId ][request.to].push(message);
			
			if (this.messages[ tabId ][request.to].length > this.maxQueueSize) {
				this.messages[ tabId ][request.to].shift();
			}
