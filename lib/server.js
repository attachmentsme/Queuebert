...
(function() {
	...
	Server.prototype.handleMessage = function(request, sender, callback) {
		...
		this.registerClient(request._for, sender);
		
		if (request.type == 'put') {
			this.messages[this.getTabId(sender)][request.to].push(request.body);
		} else if(request.type == 'get') {
			_callback(this.messages[this.getTabId(sender)][request._for].shift());
		}
	};
